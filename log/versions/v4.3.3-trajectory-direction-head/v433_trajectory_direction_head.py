...
DEFAULT_V4_CONFIG = CONFIGS_DIR / "v433_trajectory_direction_head.yaml"
VERSION = "v4.3.3"
...
def train_one(args):
    ...
    for epoch in range(int(cfg.epochs)):
        epoch_losses = []
        pbar = tqdm.tqdm(train_loader, desc=f"v433-epoch-{epoch}", unit="batch")
        ...
        write_csv(RESULTS_DIR / "v433_loss_records.csv", loss_rows)
        write_csv(RESULTS_DIR / "v433_epoch_summary.csv", epoch_rows)
    ...
    with open(RESULTS_DIR / "v433_training_summary.json", "w", encoding="utf-8") as f:
...
def command_evaluate(args):
    ...
    with open(RESULTS_DIR / "v433_validation_metrics.json", "w", encoding="utf-8") as f:
...
def command_validate_submission(args):
    ...
    with open(RESULTS_DIR / "v433_submission_validation.json", "w", encoding="utf-8") as f:
...
def command_quick_check(args):
    ...
    with open(RESULTS_DIR / "v433_quick_check.json", "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, ensure_ascii=True)
    print(json.dumps(report, indent=2))
...
def build_parser():
    ...
    p.add_argument("--threshold", type=float, default=0.9)
    p.add_argument("--inference-chunk-size", type=int, default=50000)
    p.add_argument("--name", default="v433_trajectory_direction_head_seed316_best_submission")
    p.set_defaults(func=command_submission)
    p = sub.add_parser("validate-submission")
    p.add_argument("--data-root", type=Path, default=DEFAULT_DATA_ROOT)
    p.add_argument("--txt-dir", type=Path, default=SUBMISSION_DIR / "v433_trajectory_direction_head_seed316_best_submission" / "txt")
    p.add_argument("--zip", type=Path, default=SUBMISSION_DIR / "v433_trajectory_direction_head_seed316_best_submission.zip")
