...
DEFAULT_V5_CONFIG = CONFIGS_DIR / "v500_d2_direction_head_only.yaml"
VERSION = "v5.0.0"
DIAGNOSTIC = "D2_direction_head_only"
MAX_DIRECTION_VOXELS = 1024
EXPERIMENT_NAME = "direction_head_only"
OUTPUT_PREFIX = "v500_d2"
BEST_IOU_MODEL = "best_iou_d2_seed316.pt"
BEST_LOSS_MODEL = "best_loss_d2_seed316.pt"
SEED = 316
...
def default_cfg_namespace():
    return SimpleNamespace(
        model_name="direction_head_only",
        input_channel=8,
        ...
        save=True,
        model_path=str(MODELS_DIR / BEST_IOU_MODEL),
        feature_k=32,
...
def write_yaml_config(path, cfg):
    data = {
        "STRUCTURE": {
            ...
            "bottleneck_direction_aggregation": False,
        ...
        "EXPERIMENT": {
            ...
            "bottleneck_direction_aggregation": False,
            "inference_uses_direction_aggregation": False,
            "direction_aggregation_skipped": "disabled_by_diagnostic",
            "aggregation_apply_ratio": 0.0,
        },
        "V3_FEATURES_REUSED": {
...
def command_prepare_config(args):
    ...
    cfg.seed = SEED
    cfg.model_path = str(MODELS_DIR / BEST_IOU_MODEL)
    ...
    print(json.dumps({"config": str(args.output), "version": VERSION, "experiment": EXPERIMENT_NAME, "diagnostic": DIAGNOSTIC, "input_channel": 8, "max_events_num": 50000, "optimizer": "Adam", "seed": SEED, "trajectory_projection_dice_weight": TRAJECTORY_PROJECTION_DICE_WEIGHT, "trajectory_direction_weight": TRAJECTORY_DIRECTION_WEIGHT, "direction_head_hidden": DIRECTION_HEAD_HIDDEN, "direction_neighbor_k": DIRECTION_NEIGHBOR_K, "bottleneck_direction_aggregation": False, "inference_uses_direction_aggregation": False, "direction_aggregation_skipped": "disabled_by_diagnostic", "aggregation_apply_ratio": 0.0, "trajectory_direction_head": True}, indent=2))
...
def build_model_classes():
    ...
    class BottleneckDirectionAggregation(nn.Module):
        def __init__(self, channels, max_neighbors=44, dx=1, dy=1, dt=2, time_tau=2.0, feature_temperature=0.25):
            ...
            self.skipped = True
            self.last_stats = {"direction_aggregation_skipped": "disabled_by_diagnostic", "aggregation_apply_ratio": 0.0, "avg_neighbor_count": 0.0, "bottleneck_voxels": 0}
    ...
    class TrajectoryAwareSparseUNet(nn.Module):
        def __init__(self, cfg):
            ...
            # Bottleneck: MSTB only; direction aggregation is disabled for D2.
        ...
        def forward(self, input, return_direction=False):
            ...
            self.bottleneck_dir_applied = False
            self.bottleneck_dir_agg.skipped = True
            self.bottleneck_dir_agg.last_stats = {"direction_aggregation_skipped": "disabled_by_diagnostic", "aggregation_apply_ratio": 0.0, "avg_neighbor_count": 0.0, "bottleneck_voxels": int(xb.indices.shape[0])}
...
def train_one(args):
    ...
    for epoch in range(int(cfg.epochs)):
        epoch_losses = []
        pbar = tqdm.tqdm(train_loader, desc=f"v500-d2-epoch-{epoch}", unit="batch")
        for batch_index, ev in enumerate(pbar):
            ...
            loss_rows.append({"epoch": epoch, "batch": batch_index, "loss": value, "main_loss": main_value, "trajectory_projection_dice_loss": traj_value, "trajectory_direction_loss": dir_value, "direction_cosine_loss": dir_cos_value, "direction_confidence_bce": dir_conf_value, "direction_valid_ratio": dir_stats.get("direction_valid_ratio", 0.0), "aggregation_apply_ratio": 0.0, "avg_neighbor_count": agg_stats.get("avg_neighbor_count", 0.0), "bottleneck_voxels": agg_stats.get("bottleneck_voxels", 0), "direction_aggregation_skipped": "disabled_by_diagnostic", "trajectory_projection_dice_weight": TRAJECTORY_PROJECTION_DICE_WEIGHT, "trajectory_direction_weight": TRAJECTORY_DIRECTION_WEIGHT, "lr": optimizer.param_groups[0]["lr"]})
            if value < best_loss:
                best_loss = value
                torch.save(net.state_dict(), MODELS_DIR / BEST_LOSS_MODEL)
        ...
        if epoch >= max(0, int(cfg.epochs) - int(args.eval_last_epochs)):
            ...
            if metrics["iou"] > best_iou:
                ...
                torch.save(net.state_dict(), MODELS_DIR / BEST_IOU_MODEL)
        epoch_rows.append(row)
        write_csv(RESULTS_DIR / f"{OUTPUT_PREFIX}_loss_records.csv", loss_rows)
        write_csv(RESULTS_DIR / f"{OUTPUT_PREFIX}_epoch_summary.csv", epoch_rows)
    agg_stats = getattr(net.bottleneck_dir_agg, "last_stats", {})
    direction_valid_ratio = loss_rows[-1].get("direction_valid_ratio", 0.0) if loss_rows else 0.0
    summary = {"status": "ok", "version": VERSION, "experiment": EXPERIMENT_NAME, "diagnostic": DIAGNOSTIC, "seed": SEED, "max_events_num": 50000, "input_channel": int(cfg.input_channel), "optimizer": "Adam", "activation": "LeakyReLU", "epochs": int(cfg.epochs), "best_loss": best_loss, "best_iou": best_iou, "model_path": str(MODELS_DIR / BEST_IOU_MODEL), "config": str(DEFAULT_V5_CONFIG), "trajectory_projection_dice_weight": TRAJECTORY_PROJECTION_DICE_WEIGHT, "trajectory_projection_dice_mode": TRAJECTORY_PROJECTION_DICE_MODE, "trajectory_direction_weight": TRAJECTORY_DIRECTION_WEIGHT, "initial_loss": "L_main + 0.3*L_traj + 0.1*L_dir", "direction_neighbor_k": DIRECTION_NEIGHBOR_K, "direction_head_hidden": DIRECTION_HEAD_HIDDEN, "direction_confidence_head": True, "direction_gt_note": "foreground-limited PCA; same-instance preferred when idx_label mapping is available; no background PCA fallback", "auxiliary_supervision": TRAJECTORY_PROJECTION_DICE_MODE + "+direction_confidence_head", "inference_uses_auxiliary_head": False, "bottleneck_direction_aggregation": False, "inference_uses_direction_aggregation": False, "direction_aggregation_skipped": "disabled_by_diagnostic", "aggregation_apply_ratio": 0.0, "avg_neighbor_count": agg_stats.get("avg_neighbor_count", 0.0), "direction_valid_ratio": direction_valid_ratio, "bottleneck_voxels": agg_stats.get("bottleneck_voxels", 0), "trajectory_direction_head": True}
    with open(RESULTS_DIR / f"{OUTPUT_PREFIX}_training_summary.json", "w", encoding="utf-8") as f:
...
def command_validate_submission(args):
    ...
    report = {"status": "ok" if not errors else "failed", "version": VERSION, "experiment": EXPERIMENT_NAME, "diagnostic": DIAGNOSTIC, "seed": SEED, "txt_dir": str(txt_dir), "zip": str(zip_path), "txt_count": len(txt_files), "errors": errors}
    with open(RESULTS_DIR / f"{OUTPUT_PREFIX}_submission_validation.json", "w", encoding="utf-8") as f:
...
def build_parser():
    ...
    p.add_argument("--weight", type=Path, default=MODELS_DIR / BEST_IOU_MODEL)
    p.add_argument("--threshold", type=float, default=0.9)
    ...
    p.add_argument("--threshold", type=float, default=0.9)
    p.add_argument("--inference-chunk-size", type=int, default=50000)
    p.add_argument("--name", default="v500_d2_direction_head_only_seed316_best_submission")
    p.set_defaults(func=command_submission)
    p = sub.add_parser("validate-submission")
    p.add_argument("--data-root", type=Path, default=DEFAULT_DATA_ROOT)
    p.add_argument("--txt-dir", type=Path, default=SUBMISSION_DIR / "v500_d2_direction_head_only_seed316_best_submission" / "txt")
    p.add_argument("--zip", type=Path, default=SUBMISSION_DIR / "v500_d2_direction_head_only_seed316_best_submission.zip")
