...
DEFAULT_V4_CONFIG = CONFIGS_DIR / "v600_transformer_on_v450.yaml"
VERSION = "v6.0.0"
MAX_DIRECTION_VOXELS = 1024
EXPERIMENT_NAME = "transformer_on_v450"
SEED = 316
...
TRANSFORMER_TOKEN_LIMIT = 512
TRANSFORMER_NUM_HEADS = 4
TRANSFORMER_FF_MULTIPLIER = 2
TRANSFORMER_DROPOUT = 0.0
INPUT_STATS_FILENAME = "v600_input_stats.json"
QUICK_CHECK_FILENAME = "v600_quick_check.json"
TRAINING_SUMMARY_FILENAME = "v600_training_summary.json"
VALIDATION_METRICS_FILENAME = "v600_validation_metrics.json"
THRESHOLD_SCAN_CSV = "v600_threshold_scan.csv"
THRESHOLD_SCAN_JSON = "v600_threshold_scan.json"
THRESHOLD_SCAN_MD = "v600_threshold_scan_summary.md"
SUBMISSION_VALIDATION_FILENAME = "v600_submission_validation.json"
...
def default_cfg_namespace():
    return SimpleNamespace(
        model_name="transformer_on_v450",
        input_channel=8,
...
def train_one(args):
    ...
    for epoch in range(int(cfg.epochs)):
        epoch_losses = []
        pbar = tqdm.tqdm(train_loader, desc=f"v600-epoch-{epoch}", unit="batch")
        ...
        write_csv(RESULTS_DIR / "v600_loss_records.csv", loss_rows)
        write_csv(RESULTS_DIR / "v600_epoch_summary.csv", epoch_rows)
    ...
    with open(RESULTS_DIR / "v600_training_summary.json", "w", encoding="utf-8") as f:
...
def command_evaluate(args):
    ...
    with open(RESULTS_DIR / "v600_validation_metrics.json", "w", encoding="utf-8") as f:
...
def command_validate_submission(args):
    ...
    with open(RESULTS_DIR / "v600_submission_validation.json", "w", encoding="utf-8") as f:
...
def write_threshold_svg(rows, summary):
    ...
    svg.append(f'<text x="{width/2}" y="24" text-anchor="middle" font-size="16" font-family="Arial" fill="#111">v6.0.0 Threshold Scan</text>')
    ...
    svg.append('</svg>')
    (CHARTS_DIR / "v600_threshold_scan.svg").write_text("\n".join(svg) + "\n", encoding="utf-8")
...
def command_build_comparison(args):
    ...
    rows = [
        {"version": "v6.0.0", "work_point": f"recommended@{rec['threshold']:.2f}", "label": "v6.0.0 Transformer on v4.5.0 input normalization", "source_version": "v6.0.0", "threshold": rec["threshold"], "iou": rec["iou"], "seg_acc": rec["seg_acc"], "pd": rec["pd"], "fa": rec["fa"], "tp": rec["tp"], "fp": rec["fp"], "fn": rec["fn"], "pred_positive": rec["pred_positive"], "gt_positive": rec["gt_positive"], "total_events": rec["total_events"], "max_events_num": 50000, "input_channel": 8, "optimizer": "Adam", "epochs": 50, "submission_validation_status": validation.get("status", ""), "submission_txt_count": validation.get("txt_count", ""), "notes": "Fixed train-set statistics normalize [x,y,t,p,d,L*,C,T]; bottleneck token Transformer enabled."},
        {"version": "v6.0.0", "work_point": f"best-IoU@{best['threshold']:.2f}", "label": "v6.0.0 best-IoU threshold", "source_version": "v6.0.0", "threshold": best["threshold"], "iou": best["iou"], "seg_acc": best["seg_acc"], "pd": best["pd"], "fa": best["fa"], "tp": best["tp"], "fp": best["fp"], "fn": best["fn"], "pred_positive": best["pred_positive"], "gt_positive": best["gt_positive"], "total_events": best["total_events"], "max_events_num": 50000, "input_channel": 8, "optimizer": "Adam", "epochs": 50, "submission_validation_status": "not_submitted", "submission_txt_count": "", "notes": "Highest IoU point from v6.0.0 threshold scan."},
    ]
    ...
    csv_path = CHARTS_DIR / "v600_v434_v433_v420_v313_v210_comparison.csv"
    ...
    conclusion = f"v6.0.0 recommended threshold={rec['threshold']:.2f}, IoU={rec['iou']:.6f}, FA={rec['fa']:.9f}; comparison uses the same validation protocol as v4.3.4."
    summary = {"comparison": "v6.0.0 vs v4.5.0 vs v4.3.4 vs v4.3.3 vs v4.2.0 vs v3.1.3 vs v2.1.0", "generated_at": date.today().isoformat(), "metrics_direction": {"iou": "higher_better", "seg_acc": "higher_better", "pd": "higher_better", "fa": "lower_better"}, "rows": rows, "best": best_versions, "conclusion": conclusion, "artifact_paths": {"csv": str(csv_path), "json": str(CHARTS_DIR / "v600_v434_v433_v420_v313_v210_comparison.json"), "markdown": str(CHARTS_DIR / "v600_v434_v433_v420_v313_v210_comparison.md"), "svg": str(CHARTS_DIR / "v600_v434_v433_v420_v313_v210_comparison.svg")}}
    with open(CHARTS_DIR / "v600_v434_v433_v420_v313_v210_comparison.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, ensure_ascii=True)
...
def write_comparison_markdown(rows, summary):
    lines = ["# v6.0.0 / v4.5.0 / v4.3.4 / v4.3.3 / v4.2.0 / v3.1.3 / v2.1.0 同口径对比", "", "说明：IoU、seg_acc、PD 越高越好，FA 越低越好。", "", "| 版本 | 工作点 | IoU | seg_acc | PD | FA | threshold | epochs | 提交校验 |", "|---|---|---:|---:|---:|---:|---:|---:|---|"]
    for r in rows:
    ...
    (CHARTS_DIR / "v600_v434_v433_v420_v313_v210_comparison.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_comparison_svg(rows):
    ...
    plot_h = height - 130
    svg = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">', '<rect width="100%" height="100%" fill="#ffffff"/>', f'<text x="{width/2}" y="26" text-anchor="middle" font-size="16" font-family="Arial" fill="#111">v6.0.0 same-protocol comparison</text>', f'<line x1="{margin}" y1="{height-70}" x2="{width-40}" y2="{height-70}" stroke="#333"/>']
    ...
    svg.append('</svg>')
    (CHARTS_DIR / "v600_v434_v433_v420_v313_v210_comparison.svg").write_text("\n".join(svg) + "\n", encoding="utf-8")
...
def command_quick_check(args):
    ...
    with open(RESULTS_DIR / "v600_quick_check.json", "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, ensure_ascii=True)
    print(json.dumps(report, indent=2))


def build_parser():
    parser = argparse.ArgumentParser(description="v6.0.0 Transformer on v4.5.0 input normalization experiment")
    ...
    p.add_argument("--threshold", type=float, default=0.9)
    p.add_argument("--inference-chunk-size", type=int, default=50000)
    p.add_argument("--name", default="v600_input_normalization_seed316_best_submission")
    p.set_defaults(func=command_submission)
    p = sub.add_parser("validate-submission")
    p.add_argument("--data-root", type=Path, default=DEFAULT_DATA_ROOT)
    p.add_argument("--txt-dir", type=Path, default=SUBMISSION_DIR / "v600_input_normalization_seed316_best_submission" / "txt")
    p.add_argument("--zip", type=Path, default=SUBMISSION_DIR / "v600_input_normalization_seed316_best_submission.zip")
