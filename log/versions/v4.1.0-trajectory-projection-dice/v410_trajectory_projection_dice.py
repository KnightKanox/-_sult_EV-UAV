...
DEFAULT_V4_CONFIG = CONFIGS_DIR / "v410_trajectory_projection_dice.yaml"
VERSION = "v4.1.2"
EXPERIMENT_NAME = "trajectory_projection_dice"
SEED = 316
...
def default_cfg_namespace():
    return SimpleNamespace(
        model_name="trajectory_projection_dice",
        input_channel=8,
...
def command_prepare_config(args):
    ...
    print(json.dumps({"config": str(args.output), "version": VERSION, "experiment": EXPERIMENT_NAME, "input_channel": 8, "max_events_num": 50000, "optimizer": "Adam", "seed": SEED, "trajectory_projection_dice_weight": TRAJECTORY_PROJECTION_DICE_WEIGHT}, indent=2))
...
def build_model_classes():
    ...
    class TrajectoryAwareSparseUNet(nn.Module):
        def __init__(self, cfg):
            ...
            self.bottleneck = MSTB(c4, "v400_bottleneck", norm_fn, negative_slope)
...
def train_one(args):
    ...
    for epoch in range(int(cfg.epochs)):
        epoch_losses = []
        pbar = tqdm.tqdm(train_loader, desc=f"v410-epoch-{epoch}", unit="batch")
...
def command_evaluate(args):
    ...
    metrics.update({"version": VERSION, "experiment": EXPERIMENT_NAME, "seed": SEED, "max_events_num": 50000, "inference_chunk_size": chunk_size, "input_channel": int(cfg.input_channel), "optimizer": "Adam", "activation": "LeakyReLU", "no_patchattention": True, "inference_uses_auxiliary_head": False})
    with open(RESULTS_DIR / "v410_validation_metrics.json", "w", encoding="utf-8") as f:
...
def command_validate_submission(args):
    ...
    with open(RESULTS_DIR / "v410_submission_validation.json", "w", encoding="utf-8") as f:
...
def build_parser():
    ...
    p.add_argument("--threshold", type=float, default=0.9)
    p.add_argument("--inference-chunk-size", type=int, default=50000)
    p.add_argument("--name", default="v410_trajectory_projection_dice_seed316_best_submission")
    p.set_defaults(func=command_submission)
    p = sub.add_parser("validate-submission")
    p.add_argument("--data-root", type=Path, default=DEFAULT_DATA_ROOT)
    p.add_argument("--txt-dir", type=Path, default=SUBMISSION_DIR / "v410_trajectory_projection_dice_seed316_best_submission" / "txt")
    p.add_argument("--zip", type=Path, default=SUBMISSION_DIR / "v410_trajectory_projection_dice_seed316_best_submission.zip")
