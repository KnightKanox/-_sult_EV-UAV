...
DEFAULT_V4_CONFIG = CONFIGS_DIR / "v420_bottleneck_direction_aggregation.yaml"
VERSION = "v4.2.0"
EXPERIMENT_NAME = "bottleneck_direction_aggregation"
SEED = 316
...
def default_cfg_namespace():
    return SimpleNamespace(
        model_name="bottleneck_direction_aggregation",
        input_channel=8,
...
def write_yaml_config(path, cfg):
    data = {
        ...
        "TRAIN": {
            ...
            "auxiliary_supervision": TRAJECTORY_PROJECTION_DICE_MODE,
        },
        "TEST": {
            "vis": False,
            "eval": True,
...
def command_prepare_config(args):
    ...
    print(json.dumps({"config": str(args.output), "version": VERSION, "experiment": EXPERIMENT_NAME, "input_channel": 8, "max_events_num": 50000, "optimizer": "Adam", "seed": SEED, "trajectory_projection_dice_weight": TRAJECTORY_PROJECTION_DICE_WEIGHT, "bottleneck_direction_aggregation": True}, indent=2))
...
def build_model(cfg, train=False, weight_path=None, use_cuda=True):
...
def train_one(args):
    ...
    for epoch in range(int(cfg.epochs)):
        epoch_losses = []
        pbar = tqdm.tqdm(train_loader, desc=f"v420-epoch-{epoch}", unit="batch")
        for batch_index, ev in enumerate(pbar):
            ...
            with torch.cuda.amp.autocast():
                ...
                loss = main_loss + TRAJECTORY_PROJECTION_DICE_WEIGHT * traj_loss
            ...
            loss_rows.append({"epoch": epoch, "batch": batch_index, "loss": value, "main_loss": main_value, "trajectory_projection_dice_loss": traj_value, "trajectory_projection_dice_weight": TRAJECTORY_PROJECTION_DICE_WEIGHT, "lr": optimizer.param_groups[0]["lr"]})
            if value < best_loss:
                best_loss = value
                torch.save(net.state_dict(), MODELS_DIR / "best_loss_seed316.pt")
            pbar.set_postfix(loss=value, main=main_value, traj_dice=traj_value)
            torch.cuda.empty_cache()
        scheduler.step()
...
def command_evaluate(args):
    ...
    dir_agg_skipped = getattr(net, "bottleneck_dir_agg", None) is not None and net.bottleneck_dir_agg.skipped
    metrics.update({"version": VERSION, "experiment": EXPERIMENT_NAME, "seed": SEED, "max_events_num": 50000, "inference_chunk_size": chunk_size, "input_channel": int(cfg.input_channel), "optimizer": "Adam", "activation": "LeakyReLU", "no_patchattention": True, "inference_uses_auxiliary_head": False, "bottleneck_direction_aggregation": True, "direction_aggregation_skipped": dir_agg_skipped})
    with open(RESULTS_DIR / "v420_validation_metrics.json", "w", encoding="utf-8") as f:
...
def command_validate_submission(args):
    ...
    with open(RESULTS_DIR / "v420_submission_validation.json", "w", encoding="utf-8") as f:
...
def build_parser():
    ...
    p.add_argument("--threshold", type=float, default=0.9)
    p.add_argument("--inference-chunk-size", type=int, default=50000)
    p.add_argument("--name", default="v420_bottleneck_direction_aggregation_seed316_best_submission")
    p.set_defaults(func=command_submission)
    p = sub.add_parser("validate-submission")
    p.add_argument("--data-root", type=Path, default=DEFAULT_DATA_ROOT)
    p.add_argument("--txt-dir", type=Path, default=SUBMISSION_DIR / "v420_bottleneck_direction_aggregation_seed316_best_submission" / "txt")
    p.add_argument("--zip", type=Path, default=SUBMISSION_DIR / "v420_bottleneck_direction_aggregation_seed316_best_submission.zip")
