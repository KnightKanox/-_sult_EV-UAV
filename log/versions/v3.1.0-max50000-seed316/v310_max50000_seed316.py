...
DEFAULT_V3_CONFIG = CONFIGS_DIR / "v310_max50000_seed316.yaml"
SEED = 316
...
def train_one(args):
    ...
    for epoch in range(int(cfg.epochs)):
        epoch_losses = []
        pbar = tqdm.tqdm(train_loader, desc=f"v310-epoch-{epoch}", unit="batch")
        ...
        write_csv(RESULTS_DIR / "v310_loss_records.csv", loss_rows)
        write_csv(RESULTS_DIR / "v310_epoch_summary.csv", epoch_rows)
    summary = {"status": "ok", "version": "v3.1.0", "seed": SEED, "max_events_num": int(cfg.max_events_num), "input_channel": int(cfg.input_channel), "epochs": int(cfg.epochs), "best_loss": best_loss, "best_iou": best_iou, "model_path": str(MODELS_DIR / "best_iou_seed316.pt"), "config": str(DEFAULT_V3_CONFIG)}
    with open(RESULTS_DIR / "v310_training_summary.json", "w", encoding="utf-8") as f:
...
def command_evaluate(args):
    ...
    metrics.update({"version": "v3.1.0", "seed": SEED, "max_events_num": int(getattr(cfg, "max_events_num", 50000)), "input_channel": int(cfg.input_channel)})
    with open(RESULTS_DIR / "v310_validation_metrics.json", "w", encoding="utf-8") as f:
...
def command_validate_submission(args):
    ...
    report = {"status": "ok" if not errors else "failed", "version": "v3.1.0", "seed": SEED, "txt_dir": str(txt_dir), "zip": str(zip_path), "txt_count": len(txt_files), "errors": errors}
    with open(RESULTS_DIR / "v310_submission_validation.json", "w", encoding="utf-8") as f:
...
def command_quick_check(args):
    ...
    report = {"version": "v3.1.0", "seed": SEED, "max_events_num": int(getattr(cfg, "max_events_num", 50000)), "sample": str(sample), "rows": rows, "feature_shape": list(features.shape), "finite": bool(np.isfinite(features).all()), "min": features.min(axis=0).round(6).tolist(), "max": features.max(axis=0).round(6).tolist(), "seconds": round(time.time() - start, 3)}
    with open(RESULTS_DIR / "v310_quick_check.json", "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, ensure_ascii=True)
    print(json.dumps(report, indent=2))


def build_parser():
    parser = argparse.ArgumentParser(description="v3.1.0 max50000 seed316 handcrafted-feature EV-SpSegNet")
    ...
    p.add_argument("--weight", type=Path, default=MODELS_DIR / "best_iou_seed316.pt")
    p.add_argument("--threshold", type=float, default=0.9)
    p.add_argument("--name", default="v310_max50000_seed316_best_submission")
    p.set_defaults(func=command_submission)
    p = sub.add_parser("validate-submission")
    p.add_argument("--data-root", type=Path, default=DEFAULT_DATA_ROOT)
    p.add_argument("--txt-dir", type=Path, default=SUBMISSION_DIR / "v310_max50000_seed316_best_submission" / "txt")
    p.add_argument("--zip", type=Path, default=SUBMISSION_DIR / "v310_max50000_seed316_best_submission.zip")
