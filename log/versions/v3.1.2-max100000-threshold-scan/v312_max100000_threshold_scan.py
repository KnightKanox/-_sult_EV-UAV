...
DEFAULT_V3_CONFIG = CONFIGS_DIR / "v312_max100000_seed316.yaml"
SEED = 316
...
def command_prepare_config(args):
    ...
    print(json.dumps({"config": str(args.output), "input_channel": 8, "max_events_num": int(cfg.max_events_num), "seed": SEED, "model_save_root": str(MODELS_DIR)}, indent=2))
...
def train_one(args):
    ...
    for epoch in range(int(cfg.epochs)):
        epoch_losses = []
        pbar = tqdm.tqdm(train_loader, desc=f"v312-epoch-{epoch}", unit="batch")
        ...
        write_csv(RESULTS_DIR / "v312_loss_records.csv", loss_rows)
        write_csv(RESULTS_DIR / "v312_epoch_summary.csv", epoch_rows)
    summary = {"status": "ok", "version": "v3.1.2", "seed": SEED, "max_events_num": int(cfg.max_events_num), "input_channel": int(cfg.input_channel), "epochs": int(cfg.epochs), "best_loss": best_loss, "best_iou": best_iou, "model_path": str(MODELS_DIR / "best_iou_seed316.pt"), "config": str(DEFAULT_V3_CONFIG)}
    with open(RESULTS_DIR / "v312_training_summary.json", "w", encoding="utf-8") as f:
...
def command_evaluate(args):
    ...
    metrics.update({"version": "v3.1.2", "seed": SEED, "max_events_num": int(getattr(cfg, "max_events_num", 100000)), "input_channel": int(cfg.input_channel)})
    with open(RESULTS_DIR / "v312_validation_metrics.json", "w", encoding="utf-8") as f:
...
def command_validate_submission(args):
    ...
    report = {"status": "ok" if not errors else "failed", "version": "v3.1.2", "seed": SEED, "txt_dir": str(txt_dir), "zip": str(zip_path), "txt_count": len(txt_files), "errors": errors}
    with open(RESULTS_DIR / "v312_submission_validation.json", "w", encoding="utf-8") as f:
...
def parse_thresholds(args):
    if args.thresholds:
        return [float(item.strip()) for item in args.thresholds.split(",") if item.strip()]
    start = int(round(float(args.start) * 100))
    end = int(round(float(args.end) * 100))
    step = int(round(float(args.step) * 100))
    if step <= 0:
        raise ValueError("threshold step must be positive")
    return [value / 100.0 for value in range(start, end + 1, step)]
...
def command_threshold_scan(args):
    ...
    csv_path = RESULTS_DIR / "v312_threshold_scan.csv"
    write_csv(csv_path, scan_rows)
    best_iou = max(scan_rows, key=lambda item: item["iou"]) if scan_rows else None
    lowest_fa = min(scan_rows, key=lambda item: item["fa"]) if scan_rows else None
    balanced = max(scan_rows, key=lambda item: (item["pd"] - item["fa"], item["iou"])) if scan_rows else None
    summary = {
        "status": "ok",
        "version": "v3.1.2",
        "seed": SEED,
        "max_events_num": int(getattr(cfg, "max_events_num", 100000)),
        "input_channel": int(cfg.input_channel),
        "weight": str(args.weight),
        "threshold_count": len(scan_rows),
        "csv": str(csv_path),
        "best_iou": best_iou,
        "lowest_fa": lowest_fa,
        "pd_fa_tradeoff": balanced,
    }
    with open(RESULTS_DIR / "v312_threshold_scan_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, ensure_ascii=True)
    print(json.dumps(summary, indent=2))
...
def command_quick_check(args):
    ...
    report = {"version": "v3.1.2", "seed": SEED, "max_events_num": int(getattr(cfg, "max_events_num", 100000)), "sample": str(sample), "rows": rows, "feature_shape": list(features.shape), "finite": bool(np.isfinite(features).all()), "min": features.min(axis=0).round(6).tolist(), "max": features.max(axis=0).round(6).tolist(), "seconds": round(time.time() - start, 3)}
    with open(RESULTS_DIR / "v312_quick_check.json", "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, ensure_ascii=True)
    print(json.dumps(report, indent=2))


def build_parser():
    parser = argparse.ArgumentParser(description="v3.1.2 max100000 threshold-scan seed316 handcrafted-feature EV-SpSegNet")
    ...
    p.add_argument("--max-events-num", type=int, default=100000)
    ...
    p.add_argument("--weight", type=Path, default=MODELS_DIR / "best_iou_seed316.pt")
    p.add_argument("--threshold", type=float, default=0.9)
    p.add_argument("--name", default="v312_max100000_seed316_best_submission")
    p.set_defaults(func=command_submission)
    p = sub.add_parser("threshold-scan", parents=[common])
    p.add_argument("--weight", type=Path, default=MODELS_DIR / "best_iou_seed316.pt")
    p.add_argument("--start", type=float, default=0.70)
    ...
    p.add_argument("--data-root", type=Path, default=DEFAULT_DATA_ROOT)
    p.add_argument("--txt-dir", type=Path, default=SUBMISSION_DIR / "v312_max100000_seed316_best_submission" / "txt")
    p.add_argument("--zip", type=Path, default=SUBMISSION_DIR / "v312_max100000_seed316_best_submission.zip")
