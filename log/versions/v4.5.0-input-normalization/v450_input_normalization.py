...
DEFAULT_V4_CONFIG = CONFIGS_DIR / "v450_input_normalization.yaml"
VERSION = "v4.5.0"
...
INPUT_STATS_FILENAME = "v450_input_stats.json"
QUICK_CHECK_FILENAME = "v450_quick_check.json"
TRAINING_SUMMARY_FILENAME = "v450_training_summary.json"
VALIDATION_METRICS_FILENAME = "v450_validation_metrics.json"
THRESHOLD_SCAN_CSV = "v450_threshold_scan.csv"
THRESHOLD_SCAN_JSON = "v450_threshold_scan.json"
THRESHOLD_SCAN_MD = "v450_threshold_scan_summary.md"
SUBMISSION_VALIDATION_FILENAME = "v450_submission_validation.json"
...
def train_one(args):
    ...
    for epoch in range(int(cfg.epochs)):
        epoch_losses = []
        pbar = tqdm.tqdm(train_loader, desc=f"v450-epoch-{epoch}", unit="batch")
        ...
        write_csv(RESULTS_DIR / "v450_loss_records.csv", loss_rows)
        write_csv(RESULTS_DIR / "v450_epoch_summary.csv", epoch_rows)
    ...
    with open(RESULTS_DIR / "v450_training_summary.json", "w", encoding="utf-8") as f:
...
def command_evaluate(args):
    ...
    with open(RESULTS_DIR / "v450_validation_metrics.json", "w", encoding="utf-8") as f:
...
def command_validate_submission(args):
    ...
    with open(RESULTS_DIR / "v450_submission_validation.json", "w", encoding="utf-8") as f:
...
def write_threshold_svg(rows, summary):
    ...
    svg.append(f'<text x="{width/2}" y="24" text-anchor="middle" font-size="16" font-family="Arial" fill="#111">v4.5.0 Threshold Scan</text>')
    ...
    svg.append('</svg>')
    (CHARTS_DIR / "v450_threshold_scan.svg").write_text("\n".join(svg) + "\n", encoding="utf-8")
...
def command_build_comparison(args):
    ...
    rows = [
        {"version": "v4.5.0", "work_point": f"recommended@{rec['threshold']:.2f}", "label": "v4.5.0 fixed train-set input normalization", "source_version": "v4.5.0", "threshold": rec["threshold"], "iou": rec["iou"], "seg_acc": rec["seg_acc"], "pd": rec["pd"], "fa": rec["fa"], "tp": rec["tp"], "fp": rec["fp"], "fn": rec["fn"], "pred_positive": rec["pred_positive"], "gt_positive": rec["gt_positive"], "total_events": rec["total_events"], "max_events_num": 50000, "input_channel": 8, "optimizer": "Adam", "epochs": 50, "submission_validation_status": validation.get("status", ""), "submission_txt_count": validation.get("txt_count", ""), "notes": "Fixed train-set statistics normalize [x,y,t,p,d,L*,C,T]."},
        {"version": "v4.5.0", "work_point": f"best-IoU@{best['threshold']:.2f}", "label": "v4.5.0 best-IoU threshold", "source_version": "v4.5.0", "threshold": best["threshold"], "iou": best["iou"], "seg_acc": best["seg_acc"], "pd": best["pd"], "fa": best["fa"], "tp": best["tp"], "fp": best["fp"], "fn": best["fn"], "pred_positive": best["pred_positive"], "gt_positive": best["gt_positive"], "total_events": best["total_events"], "max_events_num": 50000, "input_channel": 8, "optimizer": "Adam", "epochs": 50, "submission_validation_status": "not_submitted", "submission_txt_count": "", "notes": "Highest IoU point from v4.5.0 threshold scan."},
    ]
    ...
    for row in rows:
        row["delta_iou_vs_v450"] = row["iou"] - base_iou if row["iou"] != "" else ""
        row["delta_seg_acc_vs_v450"] = row["seg_acc"] - base_seg if row["seg_acc"] != "" else ""
        row["delta_pd_vs_v450"] = row["pd"] - base_pd if row["pd"] != "" else ""
        row["delta_fa_vs_v450"] = row["fa"] - base_fa if row["fa"] != "" else ""
    csv_path = CHARTS_DIR / "v450_v434_v433_v420_v313_v210_comparison.csv"
    ...
    conclusion = f"v4.5.0 recommended threshold={rec['threshold']:.2f}, IoU={rec['iou']:.6f}, FA={rec['fa']:.9f}; comparison uses the same validation protocol as v4.3.4."
    summary = {"comparison": "v4.5.0 vs v4.3.4 vs v4.3.3 vs v4.2.0 vs v3.1.3 vs v2.1.0", "generated_at": date.today().isoformat(), "metrics_direction": {"iou": "higher_better", "seg_acc": "higher_better", "pd": "higher_better", "fa": "lower_better"}, "rows": rows, "best": best_versions, "conclusion": conclusion, "artifact_paths": {"csv": str(csv_path), "json": str(CHARTS_DIR / "v450_v434_v433_v420_v313_v210_comparison.json"), "markdown": str(CHARTS_DIR / "v450_v434_v433_v420_v313_v210_comparison.md"), "svg": str(CHARTS_DIR / "v450_v434_v433_v420_v313_v210_comparison.svg")}}
    with open(CHARTS_DIR / "v450_v434_v433_v420_v313_v210_comparison.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, ensure_ascii=True)
...
def write_comparison_markdown(rows, summary):
    lines = ["# v4.5.0 / v4.3.4 / v4.3.3 / v4.2.0 / v3.1.3 / v2.1.0 同口径对比", "", "说明：IoU、seg_acc、PD 越高越好，FA 越低越好。", "", "| 版本 | 工作点 | IoU | seg_acc | PD | FA | threshold | epochs | 提交校验 |", "|---|---|---:|---:|---:|---:|---:|---:|---|"]
    for r in rows:
    ...
    (CHARTS_DIR / "v450_v434_v433_v420_v313_v210_comparison.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_comparison_svg(rows):
    ...
    plot_h = height - 130
    svg = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">', '<rect width="100%" height="100%" fill="#ffffff"/>', f'<text x="{width/2}" y="26" text-anchor="middle" font-size="16" font-family="Arial" fill="#111">v4.5.0 same-protocol comparison</text>', f'<line x1="{margin}" y1="{height-70}" x2="{width-40}" y2="{height-70}" stroke="#333"/>']
    ...
    svg.append('</svg>')
    (CHARTS_DIR / "v450_v434_v433_v420_v313_v210_comparison.svg").write_text("\n".join(svg) + "\n", encoding="utf-8")
...
def build_parser():
    ...
    p.add_argument("--threshold", type=float, default=0.9)
    p.add_argument("--inference-chunk-size", type=int, default=50000)
    p.add_argument("--name", default="v450_input_normalization_seed316_best_submission")
    p.set_defaults(func=command_submission)
    p = sub.add_parser("validate-submission")
    p.add_argument("--data-root", type=Path, default=DEFAULT_DATA_ROOT)
    p.add_argument("--txt-dir", type=Path, default=SUBMISSION_DIR / "v450_input_normalization_seed316_best_submission" / "txt")
    p.add_argument("--zip", type=Path, default=SUBMISSION_DIR / "v450_input_normalization_seed316_best_submission.zip")
