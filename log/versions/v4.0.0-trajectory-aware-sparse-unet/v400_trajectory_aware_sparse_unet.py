...
DEFAULT_V4_CONFIG = CONFIGS_DIR / "v400_trajectory_aware_sparse_unet.yaml"
VERSION = "v4.0.0"
EXPERIMENT_NAME = "trajectory_aware_sparse_unet"
SEED = 316
...
def default_cfg_namespace():
    return SimpleNamespace(
        model_name="trajectory_aware_sparse_unet",
        input_channel=8,
...
def command_prepare_config(args):
    ...
    print(json.dumps({"config": str(args.output), "version": VERSION, "input_channel": 8, "max_events_num": 50000, "optimizer": "Adam", "seed": SEED}, indent=2))
...
def train_one(args):
    ...
    for epoch in range(int(cfg.epochs)):
        epoch_losses = []
        pbar = tqdm.tqdm(train_loader, desc=f"v400-epoch-{epoch}", unit="batch")
        ...
        write_csv(RESULTS_DIR / "v400_loss_records.csv", loss_rows)
        write_csv(RESULTS_DIR / "v400_epoch_summary.csv", epoch_rows)
    summary = {"status": "ok", "version": VERSION, "seed": SEED, "max_events_num": 50000, "input_channel": int(cfg.input_channel), "optimizer": "Adam", "activation": "LeakyReLU", "epochs": int(cfg.epochs), "best_loss": best_loss, "best_iou": best_iou, "model_path": str(MODELS_DIR / "best_iou_seed316.pt"), "config": str(DEFAULT_V4_CONFIG)}
    with open(RESULTS_DIR / "v400_training_summary.json", "w", encoding="utf-8") as f:
...
def command_evaluate(args):
    ...
    metrics.update({"version": VERSION, "seed": SEED, "max_events_num": 50000, "inference_chunk_size": chunk_size, "input_channel": int(cfg.input_channel), "optimizer": "Adam", "activation": "LeakyReLU", "no_patchattention": True})
    with open(RESULTS_DIR / "v400_validation_metrics.json", "w", encoding="utf-8") as f:
...
def command_validate_submission(args):
    ...
    report = {"status": "ok" if not errors else "failed", "version": VERSION, "seed": SEED, "txt_dir": str(txt_dir), "zip": str(zip_path), "txt_count": len(txt_files), "errors": errors}
    with open(RESULTS_DIR / "v400_submission_validation.json", "w", encoding="utf-8") as f:
...
def command_quick_check(args):
    ...
    report = {"version": VERSION, "seed": SEED, "max_events_num": 50000, "input_channel": int(cfg.input_channel), "optimizer": "Adam", "activation": "LeakyReLU", "model_class": type(net).__name__, "channel_schedule": net.channel_schedule, "downsample_strides": net.downsample_strides, "no_patchattention": bool(net.no_patchattention), "temporal_weight_sums": time_weight_sums, "temporal_weights_sum_to_one": all(abs(value - 1.0) < 1e-6 for value in time_weight_sums.values()), "features": feature_report, "seconds": round(time.time() - start, 3)}
    with open(RESULTS_DIR / "v400_quick_check.json", "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, ensure_ascii=True)
    print(json.dumps(report, indent=2))


def build_parser():
    parser = argparse.ArgumentParser(description="v4.0.0 trajectory-aware sparse U-Net with gated geometry fusion and MSTB")
    ...
    p.add_argument("--threshold", type=float, default=0.9)
    p.add_argument("--inference-chunk-size", type=int, default=50000)
    p.add_argument("--name", default="v400_trajectory_aware_sparse_unet_seed316_best_submission")
    p.set_defaults(func=command_submission)
    p = sub.add_parser("validate-submission")
    p.add_argument("--data-root", type=Path, default=DEFAULT_DATA_ROOT)
    p.add_argument("--txt-dir", type=Path, default=SUBMISSION_DIR / "v400_trajectory_aware_sparse_unet_seed316_best_submission" / "txt")
    p.add_argument("--zip", type=Path, default=SUBMISSION_DIR / "v400_trajectory_aware_sparse_unet_seed316_best_submission.zip")
