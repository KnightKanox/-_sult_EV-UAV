...
class Shortcut(nn.Module):
    def __init__(self, in_channels, out_channels,norm_fn, stride=1):
        ...
        self.conv = spconv.SparseSequential(
            spconv.SubMConv3d(in_channels, out_channels, kernel_size=1, padding=1, bias=False, algo=SPCONV_ALGO),
...
class GDConv(SparseModule):
    def __init__(self, out_channels, norm_fn, stride=1, dilations=[1,2,3,4],indice_key=None):
        ...
        convs = []
        for d in dilations:
            convs.append(spconv.SubMConv3d(temp, temp, kernel_size=3, padding=d, dilation=d,stride=stride, algo=SPCONV_ALGO))
...
class GDBlock(SparseModule):
    def __init__(self, in_channels, out_channels,kernel_size, stride, norm_fn, dilations=[1,2,3,4],indice_key=None,bias=False,ad_channels=16):
        ...
        self.conv3=spconv.SparseSequential(
            spconv.SubMConv3d(out_channels+add_channel, out_channels, kernel_size=1, padding=1, bias=False, algo=SPCONV_ALGO),
            norm_fn(out_channels),
        )
