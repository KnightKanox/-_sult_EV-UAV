...
SPCONV_ALGO = spconv.ops.ConvAlgo.Native
...
class patch_attention(spconv.SparseModule,):
    def __init__(self, channel,in_sptial_size,att_sptial_size=(11,9,8),indice_key='pa1'):
        ...
        for i in range(int(maxpool_num)):
            self.layers.append(spconv.SparseMaxPool3d((2,2,4),stride=(2,2,4),indice_key=indice_key+str(i), algo=SPCONV_ALGO))

        self.multihead_attn=nn.MultiheadAttention(embed_dim=channel,num_heads=1)
        self.inver_layer=nn.ModuleList()
        for i in range(int(maxpool_num)):
            self.inver_layer.append(spconv.SparseInverseConv3d(channel, channel, (2,2,4), indice_key=indice_key+str(int(maxpool_num-1)-i), bias=False, algo=SPCONV_ALGO))
        self.conv = spconv.SubMConv3d(channel, channel, 1, stride=1, padding=1, bias=False, algo=SPCONV_ALGO)
...
def post_act_block(in_channels, out_channels, kernel_size, indice_key=None, stride=1, padding=0,
                   conv_type='subm', norm_fn=None, algo=SPCONV_ALGO,dilations=[1,2,3,4],ad_channels=16):
